...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        self.alphas_cumprod_prev = torch.cat([torch.tensor([1]).cuda(), self.alphas_cumprod_prev[:-1]])
        ...
        self.posterior_mean_coef2 = (torch.sqrt(self.alphas_cumprod) * (1 - self.alphas_cumprod_prev)) / denom
    ...
    def get_posterior_parameters(self, x_0, x_t, t):
        ...
        x_coeff_1 = extract(self.posterior_mean_coef1, t, x_0)
        x_coeff_2 = extract(self.posterior_mean_coef2, t, x_t)
    ...
    def model_predictions(self, x_t, t):
        ...
        coeff_1 = extract(self.x_0_pred_coef_1, t, x_t)
        coeff_2 = extract(self.x_0_pred_coef_2, t, pred_noise)
    ...
    def sample_times(self, total_timesteps, sampling_timesteps):
        ...
        return torch.linspace(0 , total_timesteps, sampling_timesteps).tolist()

    def get_time_pairs(self, times):
        # TODO 3.2: Generate a list of adjacent time pairs to sample from.
        pairs = [(t, t - 1) for t in times]
        return pairs
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        z = torch.normal(mean=0., std=1., size=mu_tau_i.size())
